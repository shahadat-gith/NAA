...
const Exams = () => {
  ...
  const openAddRoutine = () => {
    navigate("/actions?type=ExamRoutine");
  };

  const openEditRoutine = (routine) => {
    navigate("/actions?type=ExamRoutine", { state: { initialData: routine } });
  };

  const openPreviewRoutine = (routine) => {
    navigate("/actions?type=PreviewRoutine", {
      state: { routine, examDetails: currentExam, authorities },
    });
  };

  const openCurrentExamUpdate = () => {
    navigate("/actions?type=CurrentExamUpdate", {
      state: { initialData: currentExam },
    });
  };

  const openAdmitCardDownload = () => {
    navigate("/actions?type=AdmitCardDownload");
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[var(--text-secondary)] mt-1">
              Manage routines and admit cards
            </p>
          </div>

          <div className="flex gap-3 mt-4 md:mt-0">
            <Button variant="primary" onClick={openAddRoutine}>
              Add Schedule
            </Button>

            <Button variant="secondary" onClick={openAdmitCardDownload}>
              Admit Card
            </Button>
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-3xl p-6 mb-8">
          <div className="flex items-center justify-between">
            ...
            <Button variant="primary" onClick={openCurrentExamUpdate}>
              Update
            </Button>
          </div>
